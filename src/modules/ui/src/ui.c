...
BOOL SaveImg(wchar_t *fname, HBITMAP hBitmap) {
	...
	GUID fileformat = GetWICContainerFormatFromExtension(fname, &fmt);
	BOOL result;
	...
	if (SUCCEEDED(ui_factory->lpVtbl->CreateStream(ui_factory, &pStream))) {
		if (SUCCEEDED(ui_factory->lpVtbl->CreateBitmapFromHBITMAP(ui_factory, hBitmap, NULL, WICBitmapUseAlpha, &pWICBitmap))) {
			if (SUCCEEDED(pStream->lpVtbl->InitializeFromFilename(pStream, fname, GENERIC_WRITE)))
				if (SUCCEEDED(ui_factory->lpVtbl->CreateEncoder(ui_factory, &fileformat, NULL, &pEncoder))) {
...
LUA_METHOD(ui, dirdialog) {
	...
	wchar_t *title = lua_gettop(L) ? lua_towstring(L, 1) : NULL;
	wchar_t *path = lua_gettop(L) == 2 ? luaL_checkDirname(L, 2) : NULL;
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr))
			pFileDialog->lpVtbl->SetOptions(pFileDialog, dwOptions | FOS_PICKFOLDERS);
		if (title) {
			hr = pFileDialog->lpVtbl->SetTitle(pFileDialog, title);
			free(title);
		}
		if (path) {
			hr = SHCreateItemFromParsingName((PCWSTR)path, NULL, &IID_IShellItem, (void**)&pCurFolder);
			if (SUCCEEDED(hr))
    			pFileDialog->lpVtbl->SetFolder(pFileDialog, pCurFolder);
		}
		hr = pFileDialog->lpVtbl->Show(pFileDialog, GetActiveWindow());
		if (path) {
...
int do_update(lua_State *L) {
	...
	int maxMessages = max(3, min(20, lua_taskcount() * 2));
	while (processed < maxMessages && PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
		processed++;
		int n = lua_gettop(L);
	...
	push_params:									lua_pushinteger(L, msg.wParam/dpi);
				...
				} else lua_pushinstance(L, Task, 1);
...
LUA_PROPERTY_GET(ui, task) {
	if (uitask->taskref != LUA_NOREF)
	lua_rawgeti(L, LUA_REGISTRYINDEX, uitask->taskref);
	else lua_pushnil(L);
	return 1;
}
LUA_PROPERTY_GET(ui, mainWindow) {
	if (main)
		lua_rawgeti(L, LUA_REGISTRYINDEX, main->ref);
	else lua_pushnil(L);
	return 1;
}
...
LUA_METHOD(ui, fontdialog) {
	...
	if (result) {
		...
		lua_pushinteger(L, GetRValue(cf.rgbColors) << 16 | GetGValue(cf.rgbColors) << 8 | GetBValue(cf.rgbColors));
		free(cf.lpLogFont);
...
int __declspec(dllexport)  luaopen_ui(lua_State *L) {
	...
	ui_interface.lua_widgetconstructor = Widget__constructor;
	ui_interface.lua_widgetinitialize = Widget_init;
	ui_interface.lua_widgetdestructor = Widget_destructor;
	ui_interface.lua_registerwidget = widget_type_new;
	ui_interface.lua_widgetproc = WidgetProc;
	ui_interface.lua_uigetinfo = UiInfo;
